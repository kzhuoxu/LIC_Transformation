def calculate_public_seating_metrics_simplified(seating_level, plaza_level):
    ...
    bench_mapping = {0: 0, 1: 6, 2: 15}  # None, Minimal (2-10), Extensive (>10)
    plaza_mapping = {0: 0, 1: 2, 2: 4}   # None, Minimal (1-2), Extensive (3-5)
...
def calculate_mobility_metrics_simplified(bike_lane_level, bike_parking_level, bike_share_level):
    """Calculate metrics for Mobility Management without implementation level"""
    # Map categorical levels to actual values
    ...
    # Apply impacts directly (implementation factor now built into the calculations)
    bike_lane_impact = bike_lane_coverage
    bike_parking_impact = bike_parking_spots
    bike_share_impact = bike_share_stations
    
    # Calculate metrics
    pedestrian_safety = 40 + (bike_lane_impact * 0.3) + (bike_parking_impact * 0.2) + (bike_share_impact * 2.5)
    traffic_flow = 70 - (bike_lane_impact * 0.1) + (bike_share_impact * 2.5)
    business_access = 50 + (bike_parking_impact * 0.5) + (bike_share_impact * 4)
    cost_efficiency = 90 - (bike_lane_impact * 0.2) - (bike_parking_impact * 0.3) - (bike_share_impact * 5)
    community_support = 40 + (bike_lane_impact * 0.4) + (bike_parking_impact * 0.1) + (bike_share_impact * 1.5)
    
    # Calculate other metrics
    pedestrian_activity = 30 + (bike_lane_impact / 2)
    economic_activity = 35 + (bike_share_impact * 4)
    community_engagement = 20 + (bike_parking_impact / 2)
